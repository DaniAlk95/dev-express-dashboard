...
class OnlineMapItemViewer extends CustomItemViewer {
  ...
  _onClick(row) {
    ...
    for (var i = 0; i < markers.length; i++) {
      markers[i].tooltip.isShown = markers[i].tag === row;
    }
  ...
  _updateSelection() {
    ...
    markers.forEach((marker) => {
      marker.iconSrc = this.isSelected(marker.tag)
  ...
  renderContent(element, changeExisting) {
    let markers = [],
      ...
      showMarkers =
        mode === 'Markers' ||
        mode === 'MarkersAndRoutes' ||
        this.canMasterFilter(),
    ...
    let autoAdjust = markers.length > 0 || routes.length > 0,
      options = {
        ...
        apiKey: {
          bing: `${import.meta.env.VITE_BING_MAP_API_KEY}`,
          google: `${import.meta.env.VITE_GOOGLE_MAP_API_KEY}`,
        },
        markers: markers,
        routes:
          routes.length > 0
